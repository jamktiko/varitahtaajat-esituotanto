# Projektisuunnitelma

- [Projektisuunnitelma](#projektisuunnitelma)
  - [Projektin tiedot](#projektin-tiedot)
  - [Projektin sisältö](#projektin-sisältö)
    - [Kuvaus](#kuvaus)
    - [Projektiryhmä](#projektiryhmä)
    - [Tavoite](#tavoite)
  - [Aikataulu](#aikataulu)
  - [Raportointi, kommunikointi ja dokumentointi](#raportointi-kommunikointi-ja-dokumentointi)
    - [Raportointi](#raportointi)
    - [Tuntikirjaukset](#tuntikirjaukset)
    - [Palaverikäytänteet](#palaverikäytänteet)
    - [Kommunikointitavat](#kommunikointitavat)
    - [Dokumentointi](#dokumentointi)
    - [Dokumenttien hallinta](#dokumenttien-hallinta)
  - [Riskien hallinta](#riskien-hallinta)

## Projektin tiedot

Väri Tähtääjä

Tekijät:

- Arttu höylä
- Mari Karhu
- Eemeli Latva-aho
- Rayimov Elyorbek

## Projektin sisältö

### Kuvaus

Verkkosivu jossa on 1-3 väri teemoisia haasteita

### Projektiryhmä

| Nimi             | Vastuurooli                       |
| ---------------- | --------------------------------- |
| Arttu Höylä      | Git-päällikkö                     |
| Mari Karhu       | Dokumentti-päällikkö              |
| Eemeli Latva-aho | Tehtävä-päällikkö                 |
| Rayimov Elyorbek | Julkaisu/laadunhallinta-päällikkö |

### Tavoite

Toimiva pelisivusto jossa on vähintään yksi haaste

## Aikataulu

| Tapahtuma                                      | Päivämäärä | Muut tiedot/osanottajat                               |
| ---------------------------------------------- | ---------- | ----------------------------------------------------- |
| Projekti alkaa                                 | 23.04.2026 | Ohjelmistoprojekti 1 alkaa                            |
| Idean valinta                                  | 22.04.2026 | Valitaan tuotantoon menevä projekti-idea              |
| Projektisuunitelman laatiminen                 | 23.04.2026 |                                                       |
| Projektisuunitelman esittäminen                | 23.04.2026 |                                                       |
| Sovelluksen pohja ja Haaste 1                  | 27.04.2026 |                                                       |
| Haaste 2                                       | 04.05.2026 |                                                       |
| Haaste 2 Loppu ja Haaste 3 mahdollinen Aloitus | 06.05.2026 | Arvioidaan onko tarpeeksi aikaa Haaste 3 toteutukseen |
| Projektin testaus/viimeistely alku             | 11.05.2026 |                                                       |
| Projektin tuotannon loppu                      | 12.05.2026 |                                                       |
| Projekti päättyy                               | 13.05.2026 | Ohjelmistoprojekti 1 päättyy loppu esityksiin         |

## Raportointi, kommunikointi ja dokumentointi

### Raportointi

Raportointi tapahtuu viikkoraportoinilla joka palautetaan Moodleen

### Tuntikirjaukset

Jokainen tiimiläinen hoitaa omat tuntikirjaukset ja palautuksen moodleen

### Palaverikäytänteet

Viikkopalaveri pidetään 13:30 kampuksella oletuksena kirjastossa tai R35BP05 luokassa jos on pääsy. Maria Karhu on vastuussa palaveripöytäkirjasta. Palaveripöytäkirja säilötään Tiimin Teams jaetuissa tiedostoissa mahdoolistaakseen kaikille pääsyn.

### Kommunikointitavat

Teams ryhmä ja kasvotuste

### Dokumentointi

Kaikki kurssin vaativat dokumentit

- Projektisuunnitelma
- ReadMe
- Kestäväkehitys kysely
- Loppuesittely ja ohjeistus

Lisäksi kaikki sisäiset dokumentaatiot työstämisen avustamiseen.

### Dokumenttien hallinta

Dokumentit säilytetään molemissa Git Repossa ja Tiimin Teams jaetuissa tiedostoissa. Tämä säilytys myös mahdollistaa varmuuskopionin.
Tiedostot nimetään `nimi.versio.tiedostopääte`

## Riskien hallinta

| Riskit                       | Todennäköisyys | Impakti | Minimointi                           |
| ---------------------------- | -------------- | ------- | ------------------------------------ |
| Tiedonlähteen toiminallisuus | Pieni          | Suuri   | Yritetään etsiä backup tietolähteitä |
| Tiimiläisten sairastuminen   | Pieni          | Suuri   | jos on mahdollista etätöitä          |

